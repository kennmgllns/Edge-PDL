...
DECLARE_TASK(NatRouter,            nat_router::init,          nat_router::cycle,           1000);
DECLARE_TASK(HwInteractManager,  hw_interact_manager::init,  hw_interact_manager::cycle,  100);
...
extern "C" void app_main(void)
{
  ...
  #define RUN_TASK(task, stack, priority, core)                                        \
    ...
    // /RUN_TASK(NatRouter,       4*1024,  2, 0);
    RUN_TASK(HwInteractManager, 4*1024,  2, 0);
}


//esptool.py --chip esp32 --port COM3   --baud 921600 write_flash -z 0x1000 bootloader.bin 0x8000 partition-table.bin 0x10000 app0.bin
